...
class HCCTACEDataset(Dataset):
    ...
    def __init__(
        ...
        stride: int = 48,
        augment: bool = False,
        n1_oversample_ratio: float = 8.0,  # N1 케이스를 N0 대비 몇 배 더 샘플링할지
    ):
    ...
    def _load_case(self, idx: int):
        ...
        tumor_patch_indices = [i for i, sp in enumerate(seg_patches) if sp.sum() > 0] if seg_patches is not None else []
        result = (patches, seg_patches, t_stage, n_stage, tumor_patch_indices)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        patches, seg_patches, t_stage, n_stage, tumor_indices = self._load_case(case_idx)
        if self.augment:
            if tumor_indices and random.random() < 0.7:
                p_idx = random.choice(tumor_indices)
            else:
                p_idx = random.randint(0, len(patches) - 1)
        else:
            if tumor_indices:
                p_idx = tumor_indices[len(tumor_indices) // 2]
            else:
                p_idx = len(patches) // 2
        image = patches[p_idx][np.newaxis].copy()   # (1, P, P, P)
        if self.augment:
            if n_stage == 1:
...
# ---------------------------------------------------------------------------
# Multi-Scale Pretrain Dataset
# ---------------------------------------------------------------------------

class MultiScalePretrainDataset(Dataset):
    """
    같은 볼륨을 배치마다 다른 patch size로 추출하는 multi-scale pretrain dataset.

    매 __getitem__ 호출 시 patch_sizes 중 하나를 랜덤 선택해서 추출.
    작은 patch(96) → 림프절/세밀한 텍스처 학습
    큰 patch(128)  → 종양 전체 / 넓은 anatomical context 학습

    Args:
        base_dataset: NIfTI 경로 리스트를 가진 기본 dataset
        patch_sizes:  사용할 patch size 목록 (default: [96, 128])
        strides:      각 patch size에 대응하는 stride (default: [48, 64])
        augment:      학습용 증강 여부
    """
    ...
    def __init__(
        ...
        self.patch_sizes = patch_sizes or [96, 128]
        self.strides     = strides or [48, 64]
        self.augment     = augment

        assert len(self.patch_sizes) == len(self.strides), \
            "patch_sizes와 strides 길이가 같아야 합니다"

        # patch size별 cache: {(path_idx, patch_size): patches}
        self._cache: Dict[tuple, List] = {}

    def _get_patches(self, path_idx: int, patch_size: int, stride: int) -> List:
        key = (path_idx, patch_size)
        if key not in self._cache:
            patches, _, _ = preprocess_ct(
                self.nifti_paths[path_idx], patch_size, stride
            )
            # channel dim 추가: (P,P,P) → (1,P,P,P)
            patches = [p[np.newaxis] for p in patches]
    ...
    def __getitem__(self, idx: int) -> dict:
        # 배치마다 랜덤 patch size 선택
        scale_idx  = random.randint(0, len(self.patch_sizes) - 1)
        patch_size = self.patch_sizes[scale_idx]
        stride     = self.strides[scale_idx]

        patches = self._get_patches(idx, patch_size, stride)
        p_idx   = random.randint(0, len(patches) - 1)
        ...
        return {
            'image':      torch.from_numpy(image),
            'patch_size': torch.tensor(patch_size, dtype=torch.long),
        }


def _collect_nifti_paths(root: str, pattern: str = 'imagesTr/*.nii.gz') -> List[str]:
    """디렉토리에서 NIfTI 파일 경로 수집."""
...
def build_pretrain_dataset(
    ...
    patch_size:  int = 96,
    stride:      int = 48,
    patch_sizes: Optional[List[int]] = None,   # multi-scale 사용 시
    strides:     Optional[List[int]] = None,
) -> Dataset:
    """
    Phase 1 pretraining용 통합 데이터셋 빌드.

    patch_sizes가 지정되면 MultiScalePretrainDataset 사용.
    예) patch_sizes=[96, 128], strides=[48, 64]
    """
    # multi-scale 모드
    if patch_sizes is not None and len(patch_sizes) > 1:
        all_paths = []
        if lits_root and os.path.isdir(lits_root):
            paths = _collect_nifti_paths(lits_root, 'imagesTr/*.nii.gz')
            all_paths.extend(paths)
            print(f"[dataset] LiTS (multi-scale): {len(paths)}개")
        if tcia_root and os.path.isdir(tcia_root):
            paths = _collect_nifti_paths(tcia_root)
            all_paths.extend(paths)
            print(f"[dataset] TCGA-LIHC (multi-scale): {len(paths)}개")
        if not all_paths:
            raise RuntimeError("사용 가능한 pretrain 데이터가 없습니다.")
        print(f"[dataset] Multi-scale pretrain: 총 {len(all_paths)}개 볼륨, "
              f"patch_sizes={patch_sizes}")
        return MultiScalePretrainDataset(
            all_paths, patch_sizes=patch_sizes, strides=strides, augment=True
        )

    # 단일 patch size 모드 (기존 방식)
    datasets = []
